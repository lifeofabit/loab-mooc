 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a list with all negative numbers") {
    assert(sum(List(-1,-2,-3)) === -6)
  }

  test("sum of mixed numbers") {
    assert(sum(List(-1,-4,2,3)) === 0)
  }
  ...
  test("max throws an exception if given an empty list") {
  ...
  test("max of list with repeated numbers") {
    assert(max(List(4, 8, 8, 9)) == 9)
  }
